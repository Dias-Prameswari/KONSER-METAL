      ...
      <article class="card lg:card-side bg-base-100 shadow-md overflow-hidden">
        ...
        <div class="card-body flex justify-between ">
          <div>
            ...
            <div class="text-sm mt-2"> Metode: {{ $booking->paymentType?->nama ?? '-' }} </div>
            <div class="text-sm mt-2"> Diskon: {{ number_format($booking->discount_amount, 0, ',', '.') }} </div>
            <div class="text-sm mt-2"> Total Bayar: {{ number_format($booking->final_total, 0, ',', '.') }} </div>
          </div>

          <div class="text-right">
            <div class="font-bold text-lg">Rp {{ number_format($booking->final_total, 0, ',', '.') }}</div>
            @if($booking->discount_amount > 0)
            <div class="text-xs text-gray-500">Total sebelum diskon: Rp {{ number_format($booking->total_harga, 0, ',', '.') }}</div>
            @endif
